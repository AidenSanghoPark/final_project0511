	...
	function updateBlock(d_idx, d_block, u_nick, d_name) {
		if (d_block == "Y") {
			if (confirm(u_nick + "님의 " + d_name + "을 블록해제하시겠습니까?")) {
				location.href = "designBlockUpdate.do?d_idx=" + d_idx
						+ "&d_block=N";
			}
		} else {
	...
	<table class="table table-striped">
		...
		<tbody>
			<c:if test="${empty lists}">
				<tr>
					<td colspan="4" align="center">등록된 출품작이 없습니다.</td>
				</tr>
			</c:if>
			<c:forEach var="dto" items="${lists}">
				<tr>
					...
					<td>${dto.u_nick}</td>
					<td><c:url var="contentUrl" value="designBlock.do">
							<c:param name="d_idx">${dto.d_idx}</c:param>
						</c:url> <c:choose>
							<c:when test="${dto.d_block eq 'Y'}">
								<button class="btn btn-dark" style="font-size: 70%;"
									onclick="updateBlock('${dto.d_idx}', '${dto.d_block}', '${dto.u_nick}', '${dto.d_name}')">Block</button>
							</c:when>
							<c:otherwise>
